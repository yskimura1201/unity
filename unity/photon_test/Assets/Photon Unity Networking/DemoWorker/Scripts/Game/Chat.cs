...
public class Chat : Photon.MonoBehaviour
{
    ...
    private int chatHeight = (int)400;
    ...
    void OnGUI()
    {
        ...
        //Chat input
        chatInput = GUILayout.TextField(chatInput, GUILayout.Height(80));

        //Group target buttons
        GUILayout.BeginHorizontal();
        GUI.color = Color.black; GUILayout.Label("Send to:", GUILayout.Width(100)); GUI.color = Color.white;
        if (GUILayout.Button("ALL", GUILayout.Height( 50)))
            SendChat(PhotonTargets.All);
        if (GUILayout.Button("ALLBUF", GUILayout.Height(50)))
            SendChat(PhotonTargets.AllBuffered);
        if (GUILayout.Button("OTHER", GUILayout.Height(50)))
            SendChat(PhotonTargets.Others);
        if (GUILayout.Button("OTHERBUF", GUILayout.Height(50)))
            SendChat(PhotonTargets.OthersBuffered);
        if (GUILayout.Button("MASTER", GUILayout.Height(50)))
        ...
        foreach (PhotonPlayer player in PhotonNetwork.playerList)
            if (GUILayout.Button("" + player, GUILayout.MaxWidth(150), GUILayout.Height(50)))
                SendChat(player);
        GUILayout.EndHorizontal();
